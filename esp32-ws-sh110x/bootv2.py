...
# Intentar importar sh1106 (SH1106G)
try:
    import sh1106
    OLED_AVAILABLE = True
...
# Inicializar OLED SH110X
oled_initialized = False
oled = None
if OLED_AVAILABLE:
    try:
        i2c = SoftI2C(scl=Pin(OLED_SCL_PIN), sda=Pin(OLED_SDA_PIN))
        # SH1106G usa 128x64, rotate=180 para voltear la pantalla
...
def update_oled():
    """Actualiza pantalla OLED SH110X"""
    ...
    try:
        ...
        # Línea 2: ID
        oled.text(USERNAME, 0, 10)

        # Línea 3: Temp OUT
        with data_lock:
            if current_data.ds18b20_valid:
                oled.text(f"T.OUT:{current_data.ds18b20_temp:.1f}C", 0, 20)
            else:
                oled.text("T.OUT:ERROR", 0, 20)

            # Línea 4: Temp IN
            if current_data.dht_valid:
                oled.text(f"T.IN:{current_data.dht_temp:.1f}C", 0, 30)
            else:
                oled.text("T.IN:ERROR", 0, 30)

            # Línea 5: Humedad
            if current_data.dht_valid:
                oled.text(f"Hum:{current_data.dht_humidity:.0f}%", 0, 40)
            else:
                oled.text("Hum:ERROR", 0, 40)
        ...
        if wlan and wifi_connected:
            ...
            # Texto más corto para caber en 128px
            oled.text(f"W:{barras_visual} WS:{ws_status}", 0, 50)
        else:
            oled.text("WiFi:OFF", 0, 50)
...
def main():
    ...
    last_detect = time.ticks_ms()  # ⬅️ NUEVO: timer para detección

    try:
        while True:
            now = time.ticks_ms()

            # ⬅️ NUEVO: Detectar sensores cada 5 segundos
        ...
        if oled_initialized:
            oled.fill(0)
            oled.text("Sistema", 0, 20)
            oled.text("Detenido", 0, 32)
            oled.show()
            time.sleep(1)
